...
class BackupInfo(BaseModel):
    ...
    original_config_path: Path = Field(
        ..., description="Original configuration file path"
    )
    ...
    def atomic_write_with_serializer(
        ...
        if file_path.exists() and not skip_backup:
            ...
            if not backup_result.success:
                raise BackupError(
                    f"Required backup failed: {backup_result.error_message}"
                )
...
class MCPHostConfigBackupManager:
    ...
    def __init__(self, backup_root: Optional[Path] = None):
        ...
        """
        self.backup_root = (
            backup_root or Path.home() / ".hatch" / "mcp_host_config_backups"
        )
    ...
    def create_backup(self, config_path: Path, hostname: str) -> BackupResult:
        ...
        try:
            ...
            if not self.atomic_ops.atomic_copy(config_path, backup_path):
                return BackupResult(
                    success=False, error_message="Atomic copy operation failed"
                )
            ...
            if backup_size != original_size:
                backup_path.unlink()
                return BackupResult(
                    success=False, error_message="Backup size mismatch - backup deleted"
                )
            ...
            return BackupResult(
                success=False, error_message=f"Backup creation failed: {str(e)}"
            )
                ...
                try:
                    ...
                    backup_info = BackupInfo(
                        ...
                        original_config_path=Path(
                            "placeholder"
                        ),  # Will be implemented in host config phase
                    )
...
class BackupAwareOperation:
    ...
    def prepare_backup(
        self, config_path: Path, hostname: str, no_backup: bool = False
    ...
    def rollback_on_failure(
        self, backup_result: Optional[BackupResult], config_path: Path, hostname: str
    ) -> bool:
