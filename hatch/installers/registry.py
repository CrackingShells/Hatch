...
class InstallerRegistry:
    ...
    def register_installer(
        self, dep_type: str, installer_cls: Type[DependencyInstaller]
    ) -> None:
        ...
        if not issubclass(installer_cls, DependencyInstaller):
            raise ValueError(
                f"installer_cls must be a subclass of DependencyInstaller, got {installer_cls}"
            )

        if dep_type in self._installers:
            logger.warning(
                f"Overriding existing installer for type '{dep_type}': {self._installers[dep_type]} -> {installer_cls}"
            )

        self._installers[dep_type] = installer_cls
        logger.debug(
            f"Registered installer for type '{dep_type}': {installer_cls.__name__}"
        )
    ...
    def get_installer(self, dep_type: str) -> DependencyInstaller:
        ...
        logger.debug(
            f"Created installer instance for type '{dep_type}': {installer_cls.__name__}"
        )
        return installer
    ...
    def unregister_installer(
        self, dep_type: str
    ) -> Optional[Type[DependencyInstaller]]:
        ...
        if installer_cls:
            logger.debug(
                f"Unregistered installer for type '{dep_type}': {installer_cls.__name__}"
            )
