...
class PythonEnvironmentManager:
    ...
    def __init__(self, environments_dir: Optional[Path] = None):
        ...
        self.logger.debug(
            f"Python environment manager initialized with environments_dir: {self.environments_dir}"
        )
        if self.mamba_executable:
            ...
            self.logger.warning(
                "Neither conda nor mamba found - Python environment management will be limited"
            )
    ...
    def _detect_manager(self, manager: str) -> Optional[str]:
        ...
        for path in paths:
            ...
            try:
                ...
                self.logger.warning(
                    f"{manager.capitalize()} not found or not working at {path}: {e}"
                )
        return None
    ...
    def create_python_environment(
        self, env_name: str, python_version: Optional[str] = None, force: bool = False
    ) -> bool:
        ...
        if not self.is_available():
            raise PythonEnvironmentError(
                "Neither conda nor mamba is available for Python environment management"
            )
        ...
        try:
            self.logger.debug(
                f"Creating Python environment for {env_name} with name {env_name_conda}"
            )
            if python_version:
            ...
            if result.returncode == 0:
                ...
                error_msg = "Failed to create Python environment (see terminal output)"
    ...
    def remove_python_environment(self, env_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                error_msg = "Failed to remove Python environment: (see terminal output)"
                self.logger.error(error_msg)
                raise PythonEnvironmentError(error_msg)
    ...
    def get_environment_activation_info(
        self, env_name: str
    ...
    def get_manager_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "preferred_manager": (
                self.mamba_executable
                if self.mamba_executable
                else self.conda_executable
            ),
    ...
    def get_environment_diagnostics(self, env_name: str) -> Dict[str, Any]:
        ...
        if self.environment_exists(env_name):
            ...
            diagnostics["environment_path_exists"] = (
                env_path.exists() if env_path else False
            )
    ...
    def get_manager_diagnostics(self) -> Dict[str, Any]:
        ...
        for manager_name, executable in [
            ("conda", self.conda_executable),
            ("mamba", self.mamba_executable),
        ]:
            if executable:
                try:
                    result = subprocess.run(
    ...
    def launch_shell(self, env_name: str, cmd: Optional[str] = None) -> bool:
        ...
        try:
            if cmd:
                ...
                result = subprocess.run([python_exec, "-c", cmd], cwd=os.getcwd())
                ...
                if platform.system() == "Windows":
                    ...
                    result = subprocess.run(
                        ["cmd", "/c", activate_cmd], cwd=os.getcwd()
                    )
                else:
                    # On Unix-like systems, we can directly use the Python executable
                    result = subprocess.run([python_exec], cwd=os.getcwd())
