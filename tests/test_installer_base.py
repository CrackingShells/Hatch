...
from hatch.installers.installer_base import DependencyInstaller, InstallationError
...
class BaseInstallerTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.context = InstallationContext(
            environment_path=self.env_path, environment_name="test_env"
        )
    ...
    def test_installation_context_creation(self):
        ...
        context = InstallationContext(
            environment_path=Path("/test/env"), environment_name="test_env"
        )
        self.assertEqual(
            context.environment_path,
            Path("/test/env"),
            f"Expected environment_path=/test/env, got {context.environment_path}",
        )
        self.assertEqual(
            context.environment_name,
            "test_env",
            f"Expected environment_name='test_env', got {context.environment_name}",
        )
        self.assertTrue(
            context.parallel_enabled,
            f"Expected parallel_enabled=True, got {context.parallel_enabled}",
        )  # Default value
        self.assertEqual(
            context.get_config("nonexistent", "default"),
            "default",
            f"Expected default config fallback, got {context.get_config('nonexistent', 'default')}",
        )
    ...
    def test_installation_context_with_config(self):
        ...
        self.assertEqual(
            context.get_config("custom_setting"),
            "value",
            f"Expected custom_setting='value', got {context.get_config('custom_setting')}",
        )
        self.assertEqual(
            context.get_config("missing_key", "fallback"),
            "fallback",
            f"Expected fallback for missing_key, got {context.get_config('missing_key', 'fallback')}",
        )
    ...
    def test_installation_result_creation(self):
        ...
        self.assertEqual(
            ...
            "test_package",
            f"Expected dependency_name='test_package', got {result.dependency_name}",
        )
        self.assertEqual(
            result.status,
        ...
        self.assertEqual(
            ...
            f"Expected installed_path=/env/test_package, got {result.installed_path}",
        )
        self.assertEqual(
            result.installed_version,
            "1.0.0",
            f"Expected installed_version='1.0.0', got {result.installed_version}",
        )
    ...
    def test_installation_error(self):
        ...
        self.assertEqual(
            error.message,
            "Installation failed",
            f"Expected error message 'Installation failed', got '{error.message}'",
        )
        self.assertEqual(
            error.dependency_name,
            "test_package",
            f"Expected dependency_name='test_package', got {error.dependency_name}",
        )
        self.assertEqual(
            error.error_code,
            "DOWNLOAD_FAILED",
            f"Expected error_code='DOWNLOAD_FAILED', got {error.error_code}",
        )
    ...
    def test_mock_installer_interface(self):
        ...
        # Test properties
        self.assertEqual(
            self.installer.installer_type,
            "mock",
            f"Expected installer_type='mock', got {self.installer.installer_type}",
        )
        self.assertEqual(
            self.installer.supported_schemes,
            ["test", "mock"],
            f"Expected supported_schemes=['test', 'mock'], got {self.installer.supported_schemes}",
        )
        # Test can_install
        mock_dep = {"type": "mock", "name": "test"}
        non_mock_dep = {"type": "other", "name": "test"}
        self.assertTrue(
            self.installer.can_install(mock_dep),
            f"Expected can_install to be True for {mock_dep}",
        )
        self.assertFalse(
            self.installer.can_install(non_mock_dep),
            f"Expected can_install to be False for {non_mock_dep}",
        )
    ...
    def test_mock_installer_install(self):
        ...
        self.assertEqual(
            result.installed_path,
            self.env_path / "test_package",
            f"Expected installed_path={self.env_path / 'test_package'}, got {result.installed_path}",
        )
        self.assertEqual(
            result.installed_version,
            "1.2.0",
            f"Expected installed_version='1.2.0', got {result.installed_version}",
        )
    ...
    def test_mock_installer_validation(self):
        ...
        self.assertTrue(
            self.installer.validate_dependency(valid_dep),
            f"Expected valid dependency to pass validation: {valid_dep}",
        )
        self.assertFalse(
            self.installer.validate_dependency(invalid_dep),
            f"Expected invalid dependency to fail validation: {invalid_dep}",
        )
    ...
    def test_mock_installer_get_installation_info(self):
        ...
        self.assertEqual(
            info["installer_type"],
            "mock",
            f"Expected installer_type='mock', got {info['installer_type']}",
        )
        self.assertEqual(
            info["dependency_name"],
            "test_package",
            f"Expected dependency_name='test_package', got {info['dependency_name']}",
        )
        self.assertEqual(
            info["resolved_version"],
            "1.0.0",
            f"Expected resolved_version='1.0.0', got {info['resolved_version']}",
        )
        self.assertEqual(
            info["target_path"],
            str(self.env_path),
            f"Expected target_path={self.env_path}, got {info['target_path']}",
        )
        self.assertTrue(
            info["supported"], f"Expected supported=True, got {info['supported']}"
        )
    ...
    def test_mock_installer_uninstall_not_implemented(self):
        ...
        with self.assertRaises(
            NotImplementedError,
            msg="Expected NotImplementedError for uninstall on MockInstaller",
        ):
    ...
    def test_installation_status_enum(self):
        ...
        self.assertEqual(
            ...
            "pending",
            f"Expected PENDING='pending', got {InstallationStatus.PENDING.value}",
        )
        self.assertEqual(
            InstallationStatus.IN_PROGRESS.value,
            "in_progress",
            f"Expected IN_PROGRESS='in_progress', got {InstallationStatus.IN_PROGRESS.value}",
        )
        self.assertEqual(
            InstallationStatus.COMPLETED.value,
            "completed",
            f"Expected COMPLETED='completed', got {InstallationStatus.COMPLETED.value}",
        )
        self.assertEqual(
            InstallationStatus.FAILED.value,
            "failed",
            f"Expected FAILED='failed', got {InstallationStatus.FAILED.value}",
        )
        self.assertEqual(
            InstallationStatus.ROLLED_BACK.value,
            "rolled_back",
            f"Expected ROLLED_BACK='rolled_back', got {InstallationStatus.ROLLED_BACK.value}",
        )
