...
class PackageEnvironmentTests(unittest.TestCase):
    ...
    def _create_sample_registry(self):
        ...
        registry = {
            ...
            "stats": {"total_packages": 0, "total_versions": 0},
        }
            ...
            if pkg_path.exists():
                ...
                if metadata_path.exists():
                    try:
                            ...
                            "versions": [
                                {
                                    ...
                                    "hatch_dependencies_added": [
                                        ...
                                        for dep in metadata.get("dependencies", {}).get(
                                            "hatch", []
                                        )
                                    ],
                        ...
                        raise e
        # Update stats
        registry["stats"]["total_packages"] = len(
            registry["repositories"][0]["packages"]
        )
    ...
    def test_create_environment(self):
        ...
        self.assertTrue(
            self.env_manager.environment_exists("test_env"),
            "Environment doesn't exist after creation",
        )
    ...
    def test_remove_environment(self):
        ...
        self.assertFalse(
            self.env_manager.environment_exists("test_env"),
            "Environment still exists after removal",
        )
    ...
    def test_set_current_environment(self):
        ...
        self.assertEqual(
            current_env, "test_env", "Current environment not set correctly"
        )
    ...
    def test_add_package_with_dependencies(self):
        ...
        self.assertTrue(
            base_pkg_path.exists(), f"Base package not found: {base_pkg_path}"
        )
        ...
        # Add package to environment
        result = self.env_manager.add_package_to_environment(
            str(pkg_path), "test_env", auto_approve=True  # Auto-approve for testing
        )
        ...
        self.assertIn(
            "base_pkg", package_names, "Base package missing from environment"
        )
        self.assertIn(
            "simple_dep_pkg",
            package_names,
            "Dependent package missing from environment",
        )
    ...
    def test_add_package_with_some_dependencies_already_present(self):
        ...
        self.assertTrue(
            complex_pkg_path.exists(), f"Complex package not found: {complex_pkg_path}"
        )
        ...
        for pkg_name in expected_packages:
            self.assertIn(
                pkg_name, package_names, f"Package {pkg_name} missing from environment"
            )
    ...
    def test_add_package_with_all_dependencies_already_present(self):
        ...
        self.assertTrue(
            simple_pkg_path.exists(), f"Simple package not found: {simple_pkg_path}"
        )
        ...
        self.assertEqual(
            len(packages), 2, "Unexpected number of packages in environment"
        )
        for pkg_name in expected_packages:
            self.assertIn(
                pkg_name, package_names, f"Package {pkg_name} missing from environment"
    ...
    def test_add_package_with_version_constraint_satisfaction(self):
        ...
        self.assertTrue(
            result, "Failed to add package with version constraint dependencies"
        )
    ...
    def test_add_package_with_mixed_dependency_types(
        ...
        test_loader = TestDataLoader()
        python_dep_pkg_path = (
            test_loader.packages_dir / "dependencies" / "python_dep_pkg"
        )
        self.assertTrue(
            python_dep_pkg_path.exists(),
            f"Python dependency package not found: {python_dep_pkg_path}",
        )
        ...
        self.assertIn(
            "python_dep_pkg",
            package_names,
            "Package with mixed dependencies missing from environment",
        )
        ...
        self.assertTrue(
            result,
            "Failed to add package with mixed satisfied/unsatisfied dependencies",
        )
        ...
        self.assertIn(
            "python_dep_pkg", package_names, "Originally installed package missing"
        )
        self.assertIn(
        ...
        self.assertIn(
            "requests",
            package_names,
            f"Expected 'requests' package not found in Python environment: {packages}",
        )
    ...
    def test_add_package_with_system_dependency(
        self, mock_platform, mock_apt_avail, mock_run, mock_verify
    ):
        ...
        result = self.env_manager.add_package_to_environment(
            str(system_dep_pkg_path), "test_env", auto_approve=True
        )
    ...
    def test_add_package_with_docker_dependency(self, mock_docker_avail, mock_pull):
        ...
        result = self.env_manager.add_package_to_environment(
            str(docker_dep_pkg_path), "test_env", auto_approve=True
        )
        ...
        self.assertIn(
            "docker_dep_pkg",
            package_names,
            "Docker dependency package missing from environment",
        )
    ...
    def test_create_environment_with_mcp_server_default(self):
        ...
        def mock_install(env_name, tag=None):
            ...
            package_git_url = (
                "git+https://github.com/CrackingShells/Hatch-MCP-Server.git"
            )
        ...
        try:
            ...
            self.assertEqual(
                installed_env,
                "test_mcp_default",
                "MCP server should be installed in correct environment",
            )
            self.assertIsNone(
                installed_tag, "Default installation should use no specific tag"
            )
            ...
            self.assertIn(
                expected_name,
                package_names,
                "MCP server should be installed by default with correct name syntax",
            )
    ...
    def test_create_environment_with_mcp_server_opt_out(self):
        ...
        try:
            ...
            self.assertFalse(
                install_called,
                "MCP server installation should not be called when opted out",
            )
            ...
            self.assertNotIn(
                expected_name,
                package_names,
                "MCP server should not be installed when opted out",
            )
    ...
    def test_create_environment_with_mcp_server_custom_tag(self):
        ...
        def mock_install(env_name, tag=None):
            ...
            # Simulate successful installation
            package_git_url = (
                f"git+https://github.com/CrackingShells/Hatch-MCP-Server.git@{tag}"
            )
            env_data = self.env_manager._environments[env_name]
            env_data["packages"].append(
                {
        ...
        try:
            ...
            self.assertTrue(success, "Environment creation should succeed")
            self.assertEqual(
                installed_tag, "v0.1.0", "Custom tag should be passed to installation"
            )
            ...
            self.assertEqual(
                len(mcp_packages),
                1,
                "Exactly one MCP server package should be installed with correct name syntax",
            )
            self.assertEqual(
                mcp_packages[0]["version"],
                "v0.1.0",
                "MCP server should have correct version",
            )
    ...
    def test_create_environment_no_python_no_mcp_server(self):
        ...
        try:
            ...
            self.assertFalse(
                install_called,
                "MCP server installation should not be called without Python environment",
            )
    ...
    def test_install_mcp_server_existing_environment(self):
        ...
        try:
            ...
            self.assertEqual(
                installed_tag, "v0.2.0", "Custom tag should be passed to installation"
            )
            ...
            packages = env_data.get("packages", [])
            package_names = [pkg["name"] for pkg in packages]
            expected_name = "hatch_mcp_server @ git+https://github.com/CrackingShells/Hatch-MCP-Server.git@v0.2.0"
            self.assertIn(
                expected_name,
                package_names,
                "MCP server should be installed in environment with correct name syntax",
            )
    ...
    def test_create_python_environment_only_with_mcp_wrapper(self):
        ...
        self.env_manager.python_env_manager.create_python_environment = (
            mock_create_python_env
        )
        ...
        try:
            ...
            self.assertEqual(
                installed_env,
                "test_python_only",
                "MCP wrapper should be installed in correct environment",
            )
            ...
            self.assertTrue(
                env_data.get("python_environment"),
                "Python environment flag should be set",
            )
            self.assertIsNotNone(
                env_data.get("python_env"), "Python environment info should be set"
            )
            ...
            expected_name = "hatch_mcp_server @ git+https://github.com/CrackingShells/Hatch-MCP-Server.git"
            self.assertIn(
                expected_name, package_names, "MCP wrapper should be installed"
            )
            ...
            self.assertTrue(
                success, "Python environment creation with custom tag should succeed"
            )
            self.assertEqual(
                installed_tag,
                "dev",
                "Custom tag should be passed to MCP wrapper installation",
            )
            ...
            success = self.env_manager.create_python_environment_only(
                "test_python_only", force=True, no_hatch_mcp_server=True
            )

            self.assertTrue(
                success,
                "Python environment creation without MCP wrapper should succeed",
            )
            self.assertIsNone(
                installed_env, "MCP wrapper should not be installed when opted out"
            )
            ...
            self.assertEqual(
                len(packages),
                0,
                "No packages should be installed when MCP wrapper is opted out",
            )
    ...
    def test_add_package_non_tty_auto_approve(self, mock_isatty):
        ...
        self.assertTrue(
            result,
            "Non-TTY environment should auto-approve even with auto_approve=False",
        )
    ...
    def test_environment_variable_case_variations(self):
        ...
        result = self.env_manager.add_package_to_environment(
            str(base_pkg_path), "test_env", auto_approve=False
        )

        self.assertTrue(
            result, "Environment variable 'yes' should enable auto-approval"
        )
