...
class TestNonTTYIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env_manager = HatchEnvironmentManager(
            environments_dir=Path(self.temp_dir) / "envs", simulation_mode=True
        )
    ...
    def test_environment_variable_integration(self):
        ...
        self.assertTrue(
            result, "Package addition should succeed with HATCH_AUTO_APPROVE"
        )
    ...
    def test_multiple_package_installation_non_tty(self, mock_isatty):
        ...
        if utility_pkg_path.exists():
            result2 = self.env_manager.add_package_to_environment(
                str(utility_pkg_path), "test_env", auto_approve=False
            )
    ...
    def test_environment_variable_case_insensitive_integration(self):
        ...
        result = self.env_manager.add_package_to_environment(
            str(pkg_path), "test_env", auto_approve=False
        )

        self.assertTrue(
            result, "Package addition should succeed with case-insensitive env var"
        )
    ...
    def test_invalid_environment_variable_fallback_integration(
        self, mock_input, mock_isatty
    ):
        ...
        for i, value in enumerate(valid_values):
            with self.subTest(env_value=value):
                ...
                with patch.dict(os.environ, {"HATCH_AUTO_APPROVE": value}):
                    result = self.env_manager.add_package_to_environment(
                        str(pkg_path), env_name, auto_approve=False
                    )

                    self.assertTrue(
                        result,
                        f"Package installation should succeed with env var: {value}",
                    )
