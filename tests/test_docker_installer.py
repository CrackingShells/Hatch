...
class TestDockerInstaller(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.context = DummyContext(env_path=Path(self.temp_dir), simulation_mode=False)
    ...
    @unittest.skipUnless(
        DOCKER_AVAILABLE and DOCKER_DAEMON_AVAILABLE,
    ...
    def test_validate_dependency_missing_name(self):
        ...
        dependency = {"version_constraint": ">=1.25.0", "type": "docker"}
        self.assertFalse(
            self.installer.validate_dependency(dependency),
    ...
    def test_validate_dependency_missing_version_constraint(self):
        ...
        dependency = {"name": "nginx", "type": "docker"}
        self.assertFalse(
            self.installer.validate_dependency(dependency),
    ...
    def test_resolve_docker_tag(self):
        ...
        test_cases = [
            ...
            (
                "==1.25.0",
                "1.25.0",
            ),  # Theoretically valid, but Docker works with tags and not version constraints. This is just to ensure the method can handle it.
            (
                ">=1.25.0",
                "1.25.0",
            ),  # Theoretically valid, but Docker works with tags and not version constraints. This is just to ensure the method can handle it.
    ...
    def test_install_simulation_mode(self):
        ...
        result = self.installer.install(
            dependency, simulation_context, progress_callback
        )
        self.assertEqual(
            result.status,
    ...
    def test_install_failure(self, mock_docker):
        ...
        dependency = {"name": "nginx", "version_constraint": "1.25.0", "type": "docker"}
    ...
    def test_install_invalid_dependency(self):
        ...
        with self.assertRaises(
            InstallationError,
            msg=f"Install should raise InstallationError for invalid dependency: {dependency}",
        ):
            self.installer.install(dependency, self.context)
