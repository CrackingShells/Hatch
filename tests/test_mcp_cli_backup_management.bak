...
Updated for M1.8 CLI refactoring:
- Handlers now use args: Namespace signature
- Mock paths updated to hatch.cli.cli_mcp
- MCPHostConfigBackupManager patched at source module
"""
...
# Import backup handlers from cli_mcp (M1.3.6 update)
from hatch.cli.cli_mcp import (
    handle_mcp_backup_restore, handle_mcp_backup_list, handle_mcp_backup_clean
)
from hatch.mcp_host_config.models import MCPHostType
from wobble import regression_test, integration_test

# Import BackupManager at module level for patching
    ...
    def test_backup_restore_argument_parsing(self):
        ...
        with patch('sys.argv', test_args):
            with patch('hatch.cli_hatch.HatchEnvironmentManager'):
                with patch('hatch.cli.cli_mcp.handle_mcp_backup_restore', return_value=0) as mock_handler:
                    try:
                        main()
    ...
    def test_backup_restore_invalid_host(self):
        ...
        with patch('hatch.cli_hatch.HatchEnvironmentManager') as mock_env_manager:
            with patch('builtins.print') as mock_print:
                args = Namespace(
                    env_manager=mock_env_manager.return_value,
                    host='invalid-host',
                    backup_file=None,
                    dry_run=False,
                    auto_approve=False
...
class TestMCPBackupListCommand(unittest.TestCase):
    ...
    def test_backup_list_argument_parsing(self):
        ...
        with patch('sys.argv', test_args):
            with patch('hatch.cli_hatch.HatchEnvironmentManager'):
                with patch('hatch.cli.cli_mcp.handle_mcp_backup_list', return_value=0) as mock_handler:
                    try:
                        main()
                        mock_handler.assert_called_once()
                    except SystemExit as e:
    ...
    def test_backup_list_invalid_host(self):
        ...
        with patch('builtins.print') as mock_print:
            args = Namespace(host='invalid-host', detailed=False)
            result = handle_mcp_backup_list(args)
    ...
    def test_backup_list_no_backups(self):
        ...
        with patch('hatch.mcp_host_config.backup.MCPHostConfigBackupManager') as mock_backup_class:
            ...
            with patch('builtins.print') as mock_print:
                args = Namespace(host='claude-desktop', detailed=False)
                result = handle_mcp_backup_list(args)
    ...
    def test_backup_list_detailed_output(self):
        ...
        with patch('hatch.mcp_host_config.backup.MCPHostConfigBackupManager') as mock_backup_class:
            ...
            with patch('builtins.print') as mock_print:
                args = Namespace(host='claude-desktop', detailed=True)
                result = handle_mcp_backup_list(args)
...
class TestMCPBackupCleanCommand(unittest.TestCase):
    ...
    def test_backup_clean_argument_parsing(self):
        ...
        with patch('sys.argv', test_args):
            with patch('hatch.cli_hatch.HatchEnvironmentManager'):
                with patch('hatch.cli.cli_mcp.handle_mcp_backup_clean', return_value=0) as mock_handler:
                    try:
                        main()
                        mock_handler.assert_called_once()
                    except SystemExit as e:
